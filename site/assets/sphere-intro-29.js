...
//   3. the mark shrinks to the rail's tile size and glides LEFT into the
//      sidebar's superbot slot while the hub's window ground grows in
//      around it — then the organizing interlude (user, 2026-09-09): the
//      right side of the stage fills with rule files, skills, MCP and auth
//      chips and the IDE tiles that never land in the rail, drifting in a
//      soft float. The mark sorts them across three jumps (chips into
//      stacks, tiles into a grid), leaps to the middle, inhales everything
//      in one accelerating suck, and settles back into its slot
//   4. then the OTHER app icons drop down from the mark, right there on the
//      real rail — a full replica of the rail assembling beneath the mark
...
(() => {
  ...
  // one tile builder for every rail-item in the story: the sphere's spinning
  // tiles (54 design px, inside the holder) and the interlude's floaters
  // (measured rail size, in overlay px) draw the same markup
  const buildTile = (a, size) => {
    const t = document.createElement("span");
    t.className = "rail-item" + (a.img ? " bleed" : "");
    if (a.app) t.dataset.app = a.app;
    t.title = a.name || a.app;
    t.style.cssText = "position:absolute;left:0;top:0;width:" + size + "px;height:" + size + "px;will-change:transform,opacity,filter;opacity:0;";
    ...
    if (a.img) {
      ...
      svg.style.width = Math.round(size * 0.574) + "px";
      svg.style.height = Math.round(size * 0.574) + "px";
    }
    return t;
  };
  ...
  // the interlude's cast (user, 2026-09-09): the machine's inventory as
  // floating chips — rule files, skills, MCP servers, auth grants — plus the
  // IDE tiles that spin but never land in the rail. hub-boot's chip copy.
  const CHIPS = [
    { kind: "rule", text: "no em dashes" },
    { kind: "rule", text: "ask before deploy" },
    { kind: "skill", text: "release" },
    { kind: "skill", text: "triage" },
    { kind: "mcp", text: "github" },
    { kind: "mcp", text: "slack" },
    { kind: "mcp", text: "postgres" },
    { kind: "auth", text: "slack" },
    { kind: "auth", text: "github" },
  ];
  const FLOT_TILES = [
    { name: "Windsurf", sym: "windsurf", bg: "var(--brand-windsurf)", ink: "var(--brand-windsurf-ink)" },  // green
    { name: "Zed", sym: "zed", bg: "var(--brand-zed)", ink: "var(--brand-zed-ink)" },                      // blue
    { name: "JetBrains", logo: "site/assets/intro/jetbrains.png", bg: "var(--surface)" },                  // orange → magenta
    { name: "Replit", logo: "site/assets/intro/replit.png", bg: "var(--surface)" },                        // orange
    { name: "Lovable", logo: "site/assets/intro/lovable.png", bg: "#ffffff" },                             // pink → purple
    { name: "Bolt", img: "site/assets/intro/bolt.png" },                                                   // black
  ];
  // interleave the two kinds so the pop-in reads as a scattered field
  const FLOT = [
    { chip: CHIPS[0] }, { tile: FLOT_TILES[0] }, { chip: CHIPS[2] }, { chip: CHIPS[4] },
    { tile: FLOT_TILES[1] }, { chip: CHIPS[1] }, { tile: FLOT_TILES[2] }, { chip: CHIPS[7] },
    { chip: CHIPS[3] }, { tile: FLOT_TILES[3] }, { chip: CHIPS[5] }, { chip: CHIPS[8] },
    { tile: FLOT_TILES[4] }, { chip: CHIPS[6] }, { tile: FLOT_TILES[5] },
  ];
  // the sort groups, in jump order: rules+skills first, then mcp+auth, then
  // every app tile
  const groupOf = (i) => (FLOT[i].chip && (FLOT[i].chip.kind === "rule" || FLOT[i].chip.kind === "skill") ? 0
    : FLOT[i].chip ? 1 : 2);

  // the organizing interlude: the floaters drift on per-item sines, the mark
  // sorts a group after each jump, leaps to the middle, inhales them all and
  // returns to its slot. Desktop beat only — the phone cut keeps the fast path
  const organize = async (g) => {
    if (arena.clientWidth < 560) return;
    const GLIDE = g.GLIDE;
    const T = g.T, k = g.k, cx = acx(), cy = acy();
    const HOME_TF = g.HOME_TF;
    const tf = (dy, sx, sy) => "translate(" + g.gx.toFixed(1) + "px," + (g.gy + dy).toFixed(1) + "px) scale(" + (k * sx).toFixed(4) + "," + (k * sy).toFixed(4) + ")";

    // the float zone: everything right of the parked mark, inside the window
    const zoneL = acx() + T * 1.5;
    const zoneR = arena.offsetLeft + arena.clientWidth - 26;
    const zoneT = arena.offsetTop + 26;
    const zoneB = arena.offsetTop + arena.clientHeight - 26;
    const zoneW = Math.max(140, zoneR - zoneL);

    // build the floaters at the arena center, then measure and scatter them
    const floaters = FLOT.map((f, i) => {
      const el = f.chip
        ? (() => {
            const c = document.createElement("span");
            c.style.cssText = "position:absolute;opacity:0;will-change:transform,opacity;height:26px;line-height:24px;padding:0 10px;border-radius:13px;background:var(--window);border:1px solid var(--line);color:var(--fg);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:" + Math.min(Math.round(zoneW * 0.55), 160) + "px;";
            const b = document.createElement("b");
            b.style.color = "var(--accent)";
            b.style.fontWeight = "600";
            b.textContent = f.chip.kind + ":";
            c.appendChild(b);
            c.appendChild(document.createTextNode(" " + f.chip.text));
            return c;
          })()
        : buildTile(f.tile, T);
      el.style.left = cx + "px";
      el.style.top = cy + "px";
      el.style.marginTop = f.chip ? "-13px" : (-T / 2) + "px";
      overlay.appendChild(el);
      const w = el.offsetWidth || (f.chip ? 110 : T);
      el.style.marginLeft = (-w / 2) + "px";
      const rnd = (kk) => { const x = Math.sin(i * 12.9898 + kk * 78.233) * 43758.5453; return x - Math.floor(x); };
      return {
        el, w, group: groupOf(i),
        // a messy home somewhere in the zone; base is the offset from center
        base: {
          x: (zoneL + 10 + rnd(1) * (zoneR - zoneL - 20 - w) + w / 2) - cx,
          y: (zoneT + 8 + rnd(2) * (zoneB - zoneT - 16 - (f.chip ? 26 : T)) + (f.chip ? 13 : T / 2)) - cy,
        },
        a1: 7 + rnd(3) * 9, a2: 6 + rnd(4) * 8,
        w1: 0.5 + rnd(5) * 0.7, w2: 0.4 + rnd(6) * 0.6,
        p1: rnd(7) * 6.28, p2: rnd(8) * 6.28,
        rotAmp: (rnd(9) - 0.5) * 9, w3: 0.3 + rnd(10) * 0.5, p3: rnd(11) * 6.28,
        born: (i * 55) / 1000, settled: false,
      };
    });

    // the drift: one rAF, transform/opacity only, pure function of the clock —
    // hub-boot's discipline
    let driftRaf = 0, driftT0 = null;
    const driftFrame = (ts) => {
      if (driftT0 == null) driftT0 = ts;
      const t = (ts - driftT0) / 1000;
      for (const f of floaters) {
        if (f.settled) continue;
        const x = f.base.x + Math.sin(t * f.w1 + f.p1) * f.a1;
        const y = f.base.y + Math.cos(t * f.w2 + f.p2) * f.a2;
        const grow = clamp01((t - f.born) / 0.34);
        const s = (0.55 + 0.45 * outBack(grow)) + Math.sin(t * f.w2 * 0.8 + f.p1) * 0.025;
        f.el.style.transform = "translate(" + x.toFixed(1) + "px," + y.toFixed(1) + "px) rotate(" + (Math.sin(t * f.w3 + f.p3) * f.rotAmp).toFixed(2) + "deg) scale(" + s.toFixed(3) + ")";
      }
    ...
    // a jump: apex stretch on the way up, contact squash on landing — full
    // transform keyframes so it composes with the parked position exactly
    const jump = (h) => anim(bloom, [
      { transform: tf(0, 1, 1), easing: "cubic-bezier(.3,.8,.4,1)" },
      { transform: tf(-h, 0.97, 1.08), offset: 0.44, easing: "cubic-bezier(.45,0,.7,.4)" },
      { transform: tf(0, 1.08, 0.9), offset: 0.7, easing: "ease-out" },
      { transform: tf(0, 1, 1) },
    ], { duration: 430, fill: "forwards" });

    // a group sorts: FLIP from wherever each item is drifting to its slot in
    // the tidy arrangement (the delayed neutral keyframe keeps it drifting
    // until its own tween starts)
    const sortCluster = async (grp, anchors) => {
      const list = floaters.filter((f) => f.group === grp);
      await Promise.all(list.map((f, i) => {
        f.settled = true;
        f.ax = anchors[i].x; // the sorted home, kept for the suck wave's ordering
        f.ay = anchors[i].y;
        return anim(f.el, [
          {},
          { transform: "translate(" + anchors[i].x.toFixed(1) + "px," + anchors[i].y.toFixed(1) + "px) rotate(0deg) scale(1)", opacity: 1 },
        ], { duration: 620, delay: i * 48, easing: GLIDE, fill: "both" });
      }));
      list.forEach((f) => settle(f.el));
    };

    // the tidy arrangements: a chip stack per kind-pair, a 2×3 tile grid
    const maxW = (grp) => Math.max(...floaters.filter((f) => f.group === grp).map((f) => f.w));
    const stack = (grp, left, top) => floaters.filter((f) => f.group === grp)
      .map((f, i) => ({ x: left + f.w / 2 - cx, y: top + 13 + i * 34 - cy }));
    const stackALeft = zoneR - maxW(0) - 4;
    const stackBLeft = stackALeft - maxW(1) - 16;
    const stackTop = zoneT + 10;
    const gridPitchX = T * 1.3, gridPitchY = T * 1.25;
    const gridW2 = T + gridPitchX, gridH3 = T + gridPitchY * 2;
    const gridLeft = Math.max(zoneL, stackBLeft - 16 - gridW2);
    const gridCx = gridLeft + T / 2, gridCy = stackTop + (13 * 2 + 34 * 4) / 2; // stack B's mid
    const grid = [0, 1, 2, 3, 4, 5].map((n) => ({
      x: gridCx + (n % 2) * gridPitchX - cx,
      y: gridCy + Math.floor(n / 2) * gridPitchY - cy,
    }));

    try {
      // pop the floaters in, then jump 1 → rules+skills sort
      floaters.forEach((f, i) => {
        const p = f.el.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 240, delay: i * 55, easing: "ease-out", fill: "forwards" });
        p.finished.catch(() => {}).then(() => settle(f.el));
      });
      await sleep(1250);
      await jump(T * 1.15);
      await sortCluster(0, stack(0, stackALeft, stackTop));
      await sleep(140);

      await jump(T * 1.4);
      await sortCluster(1, stack(1, stackBLeft, stackTop));
      await sleep(140);

      await jump(T * 1.75);
      await sortCluster(2, grid);
      await sleep(340);

      // leap to the middle and grow — then everything on the right gets
      // sucked in, farthest first, in one accelerating wave
      const KM = k * 2.5;
      await anim(bloom, [
        { transform: HOME_TF },
        { transform: "translate(0px,0px) scale(" + KM.toFixed(4) + ")" },
      ], { duration: 480, easing: GLIDE, fill: "forwards" });
      settle(bloom);
      await sleep(140);
      const wave = Promise.all([...floaters]
        .sort((a, b) => Math.hypot(b.ax, b.ay) - Math.hypot(a.ax, a.ay))
        .map((f, i) => anim(f.el, [
          {},
          { transform: "translate(0px,0px) rotate(0deg) scale(0.12)", opacity: 0 },
        ], { duration: 470, delay: i * 40, easing: "cubic-bezier(.5,0,.7,.2)", fill: "both" })));
      anim(bloom, [
        { transform: "translate(0px,0px) scale(" + KM.toFixed(4) + ")" },
        { transform: "translate(0px,0px) scale(" + (KM * 1.06).toFixed(4) + ")" },
      ], { duration: 900, easing: "ease-in-out", fill: "forwards" });
      await wave;
      floaters.forEach((f) => settle(f.el));
      await sleep(90);

      // the final gulp: a bright pulse as the last of it lands
      await anim(bloom, [
        { transform: "translate(0px,0px) scale(" + (KM * 1.06).toFixed(4) + ")", filter: "brightness(1)" },
        { transform: "translate(0px,0px) scale(" + (KM * 1.18).toFixed(4) + ")", filter: "brightness(1.35)", offset: 0.4, easing: "ease-out" },
        { transform: "translate(0px,0px) scale(" + KM.toFixed(4) + ")", filter: "brightness(1)" },
      ], { duration: 400, fill: "forwards" });
      settle(bloom);
      await sleep(120);

      // back to the original state: the mark glides home into its slot
      await anim(bloom, [
        { transform: "translate(0px,0px) scale(" + KM.toFixed(4) + ")" },
        { transform: HOME_TF },
      ], { duration: 460, easing: GLIDE, fill: "forwards" });
      settle(bloom);
    } finally {
  ...
  // beats 3 + 4 — the mark shrinks and glides LEFT into the sidebar's slot,
  // the window's ground grows in around it, the organizing interlude plays,
  // and then the rail's own app icons drop down from the mark. Nothing is
  // cloned and nothing crossfades: the icons that drop in ARE the sidebar's
  // icons, and the mark that lands is the same tile.svg the rail slot draws
  const showFrontend = async () => {
    ...
    // transparent until the mark glides left, the superbot slot waits for
    // the mark, and the selection pill waits for the sidebar. The stylesheet
    // gives .hub a transition list (background-color, border-color…) that
    // would turn these instant changes into a visible fade — a one-frame
    // flash of the window ground — so transitions are off while the stage
    // is set
    ...
    const T = sbR.width / Z; // the rail tile's size, overlay px
    ...
    // every rail child waits hidden (the dividers have no hidden state of
    // their own in the stylesheet, only the tiles do)
    items.forEach((el) => { el.style.opacity = "0"; });

    // beat L — the tile shrinks to the rail's size and glides LEFT into the
    // sidebar's slot while the window's ground grows in around it
    ...
    const gx = sbFx - acx(), gy = sbFy - acy();
    const HOME_TF = "translate(" + gx.toFixed(1) + "px," + gy.toFixed(1) + "px) scale(" + k.toFixed(4) + ")";
    await anim(bloom, [
      { transform: "scale(1)" },
      { transform: HOME_TF },
    ], { duration: 700, easing: GLIDE, fill: "forwards" });
    settle(bloom);
    hub.style.transition = "";
    rail.style.transition = "";
    hub.style.boxShadow = "";

    // the organizing interlude (user, 2026-09-09)
    await organize({ GLIDE, gx, gy, HOME_TF, k, T });

    // beat 3b — the rail's own app icons drop down from the mark, one at a
    // time, right on the real rail
    ...
    // landed: the mark sits exactly on its slot — same artwork, same size,
    // same pixel — so the slot takes over in the same frame with no fade
    sb.style.opacity = "1";
    bloom.remove();
    // the frontend extends: the sidebar, chat lane and panel fade into view,
    // and the selection pill takes its place beside the mark
  ...
  setTimeout(() => { handoff(); if (overlay.isConnected) dissolve(); playPhone(); }, startAt + 24000);
})();
