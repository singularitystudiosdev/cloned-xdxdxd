...
import { Mascot, mascotFrame, morphFrames, VARIANTS } from './mascot.js';
import { mountKeys } from './hub-keys.js';
import { CUTS, cutByName } from './hub-boot-cuts.js';
...
// the terminal transcript: one fact per line, counters in a padded key/value
// column, receipts with a check, so it reads as a log and not a paragraph
const FOUND = [['apps', 6], ['mcp servers', 14], ['rules', 32], ['skills', 9], ['auth sessions', 3]];
...
const DONE = [['aggregated', '1.2s'], ['apps wired', 7], ['copies', 1], ['devices synced', 2]];
...
const tourEls = TOUR.map((m) => {
  ...
  const av = m.own
    ...
    : (m.who === 'superbot'
      ? '<span class="avatar sb"><img src="brand/mark-clean.svg" alt=""></span>'
